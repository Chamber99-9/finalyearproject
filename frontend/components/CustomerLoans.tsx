...
import { LoanAccount, buildSchedule } from "@/lib/loans";
...
export function CustomerLoans() {
  ...
  const [scheduleLoan, setScheduleLoan] = useState<LoanAccount | null>(null);
  ...
  return (
    <div className="table-shell">
        ...
        {loans.map((loan) => {
          ...
          return (
            ...
            >
              ...
              {loan.penalty_due && loan.penalty_due > 0 ? (
                <p className="mt-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700">
                  Late fee due: {formatMoney(loan.penalty_due)} — added to your next EMI payment.
                </p>
              ) : null}
              {loan.status === "active" ? (
                <div className="mt-4 space-y-3">
                  ...
                  >
                    {payingId === loan.id
                      ? "Processing..."
                      : `Pay EMI ${formatMoney(loan.monthly_emi + (loan.penalty_due ?? 0))}`}
                  </button>
                  <button
                    className="w-full whitespace-nowrap rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
      ...
      {scheduleLoan ? (
        <ScheduleModal loan={scheduleLoan} onClose={() => setScheduleLoan(null)} />
      ) : null}
    </div>
  );
}

/** Modal showing the per-installment amortization schedule for a loan. */
function ScheduleModal({ loan, onClose }: { loan: LoanAccount; onClose: () => void }) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-950">Amortization schedule</h3>
            <p className="mt-0.5 text-xs text-slate-500">
              {formatMoney(loan.principal)} · {loan.interest_rate}% p.a. · {loan.tenure_months} months ·
              EMI {formatMoney(loan.monthly_emi)}
            </p>
          </div>
          <button
          ...
          <table className="min-w-full text-sm">
            <thead className="text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="py-2 pr-3 text-right text-emerald-700">Principal</th>
                <th className="py-2 pr-3 text-right text-amber-700">Interest</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((r) => (
                <tr key={r.month}>
                  <td className="py-2 pr-3 text-slate-600">{r.month}</td>
                  <td className="py-2 pr-3 text-right">{formatMoney(r.emi)}</td>
                  <td className="py-2 pr-3 text-right text-emerald-700">{formatMoney(r.principalPaid)}</td>
                  <td className="py-2 pr-3 text-right text-amber-700">{formatMoney(r.interestPaid)}</td>
                  <td className="py-2 text-right text-slate-700">{formatMoney(r.remainingBalance)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t-2 border-slate-200 font-semibold">
                ...
                <td className="py-2 pr-3 text-right">{formatMoney(loan.monthly_emi * loan.tenure_months)}</td>
                <td className="py-2 pr-3 text-right text-emerald-700">{formatMoney(loan.principal)}</td>
                <td className="py-2 pr-3 text-right text-amber-700">{formatMoney(Math.round(totalInterest))}</td>
