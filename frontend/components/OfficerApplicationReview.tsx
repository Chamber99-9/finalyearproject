...
export function OfficerApplicationReview({ applicationId }: OfficerApplicationReviewProps) {
  ...
  const ocrByDocumentId = useMemo(() => {
    const results = new Map<string, OCRResult>();
    for (const result of detail?.ocr_results ?? []) {
      results.set(result.document_id, result);
    }
    return results;
  }, [detail]);
    ...
    <section className="mx-auto max-w-7xl px-5 py-10 sm:px-6 lg:py-14">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_380px]">
        <div className="grid gap-6">
          ...
          <DocumentsCard documents={documents} ocrByDocumentId={ocrByDocumentId} />
        </div>
...
function DocumentsCard({
  ...
  ocrByDocumentId: Map<string, OCRResult>;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-4">
        {documents.length > 0 ? (
          documents.map((document) => {
            const ocrResult = ocrByDocumentId.get(document.id);
                ...
                {ocrResult ? (
                  <div className="mt-4 grid gap-3">
                    <div className="flex flex-wrap gap-2 text-xs font-semibold text-slate-600">
                      ...
                      <span className="rounded-md bg-white px-2.5 py-1">
                        {ocrResult.verified_by_user ? "Verified" : "Not verified"}
                      </span>
                    </div>
                    <pre className="max-h-44 overflow-auto whitespace-pre-wrap rounded-md bg-white p-3 text-xs leading-5 text-slate-700">
                      {ocrResult.extracted_text}
                    </pre>
                    <pre className="max-h-44 overflow-auto whitespace-pre-wrap rounded-md bg-white p-3 text-xs leading-5 text-slate-700">
                      {JSON.stringify(ocrResult.corrected_data, null, 2)}
                    </pre>
                  </div>
                ) : (
